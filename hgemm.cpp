...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < m; ++i)
    for (int j = 0; j < k; ++j)
      a[i * lda + j] = f2h( weights [ (i * lda + j) % weights_size ] );

  for (int i = 0; i < k; ++i)
    for (int j = 0; j < n; ++j)
      b[i * ldb + j] = f2h( weights [ ( i * ldb + j ) % weights_size ] );

  for (int i = 0; i < m; ++i)
    for (int j = 0; j < n; ++j)
      c[i * ldc + j] = f2h(weights [ (i * ldc + j) % weights_size ]);
  ...
  printf("hgemm , M = %lld N = %lld K = %lld , GFlops = %lf , GB/s = %lf , "
    ...
    */
    ...
    fwrite(c, sizeof(MKL_F16), m * n, outputFD);
