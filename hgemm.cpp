...
int main(int argc, char *argv[]) {
  ...
  c = (MKL_F16 *)mkl_calloc(rmaxc * cmaxc, sizeof(MKL_F16), 64);
  ...
  for (int i = 0; i < m; ++i)
    for (int j = 0; j < k; ++j)
      a[i * lda + j] = f2h(static_cast<float>(rand()) / static_cast<float>(RAND_MAX / 20));

  for (int i = 0; i < k; ++i)
    for (int j = 0; j < n; ++j)
      b[i * ldb + j] = f2h(static_cast<float>(rand()) / static_cast<float>(RAND_MAX / 20));

  for (int i = 0; i < m; ++i)
    for (int j = 0; j < n; ++j)
      c[i * ldc + j] = f2h(static_cast<float>(rand()) / static_cast<float>(RAND_MAX / 20));

  alpha = f2h(static_cast<float>(rand()) / static_cast<float>(RAND_MAX / 20));
  beta = f2h(static_cast<float>(rand()) / static_cast<float>(RAND_MAX / 20));
  ...
  /*                         A                       B                        C
   *                /                  \   /                   \  /                     \ */
  double gbytes = (sizeof(MKL_F16) * m * k + sizeof(MKL_F16) * k * n + sizeof(MKL_F16) * m * n) /
                  1e9;

  //mkl_verbose(1);
  /*                    WARM-UP EXECUTIONS                    */
  for (int count = 0 ; count < 4 ; ++count)
    cblas_hgemm(layout, transA, transB, m, n, k, alpha,
  ...
  printf("hgemm , M = %lld N = %lld K = %lld , GFlops = %lf , GB/s = %lf , "
