...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < m; ++i)
    for (int j = 0; j < k; ++j)
      a[i * lda + j] = static_cast<float>(rand()) / static_cast<float>(RAND_MAX / 5);

  for (int i = 0; i < k; ++i)
    for (int j = 0; j < n; ++j)
      b[i * ldb + j] = static_cast<float>(rand()) / static_cast<float>(RAND_MAX / 5);

  for (int i = 0; i < m; ++i)
    for (int j = 0; j < n; ++j)
      c[i * ldc + j] = static_cast<float>(rand()) / static_cast<float>(RAND_MAX / 5);
#endif
